...
void ProcessEvent(lp_id_t me, simtime_t now, unsigned event_type, const void *event_content, unsigned size, void *ptr)
{
	...
	switch(event_type) {
		...
		case SET_BEACONS_TIMER:
			...
			if(state->state & RUNNING) {
				/*
				 * This event is processed when the interval associated to the timer for beacons has to
				 * be updated
				 */
		...
		case TRANSMISSION_BEACON_STARTED:
			...
			new_pending_transmission(state, ((ctp_routing_packet *)event_content)->link_frame.gain,
			    CTP_BEACON, event_content, ((ctp_routing_packet *)event_content)->link_frame.duration);
			break;
		...
		case TRANSMISSION_DATA_PACKET_STARTED:
			...
			new_pending_transmission(state, ((ctp_data_packet *)event_content)->link_frame.gain,
			    CTP_DATA_PACKET, event_content, ((ctp_data_packet *)event_content)->link_frame.duration);
			break;
		...
		case ACK_RECEIVED:
			...
			if(state->state & RUNNING) {
				...
				state->last_packet_acked.link_frame.sink =
				    ((ctp_data_packet *)event_content)->link_frame.sink;
				state->last_packet_acked.link_frame.src =
				    ((ctp_data_packet *)event_content)->link_frame.src;
				state->last_packet_acked.link_frame.duration =
				    ((ctp_data_packet *)event_content)->link_frame.duration;
				state->last_packet_acked.link_frame.gain =
				    ((ctp_data_packet *)event_content)->link_frame.gain;
				state->last_packet_acked.payload = ((ctp_data_packet *)event_content)->payload;
				state->last_packet_acked.data_packet_frame.ETX =
				    ((ctp_data_packet *)event_content)->data_packet_frame.ETX;
				state->last_packet_acked.data_packet_frame.options =
				    ((ctp_data_packet *)event_content)->data_packet_frame.options;
				state->last_packet_acked.data_packet_frame.origin =
				    ((ctp_data_packet *)event_content)->data_packet_frame.origin;
				state->last_packet_acked.data_packet_frame.seqNo =
				    ((ctp_data_packet *)event_content)->data_packet_frame.seqNo;
				state->last_packet_acked.data_packet_frame.THL =
				    ((ctp_data_packet *)event_content)->data_packet_frame.THL;
			}
			break;
...
void print_statistics(unsigned int root)
{
	...
	for(i = 0; i < n_prc_tot; i++) {
		...
		printf("Beacons received by %d:%lu\n", i, node_statistics_list[i].beacons_received);
		printf("Beacons sent by %d:%lu\n", i, node_statistics_list[i].beacons_sent);
		printf("Data packets received by %d:%lu\n", i, node_statistics_list[i].data_packets_received);
