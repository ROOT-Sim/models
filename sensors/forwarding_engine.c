    ...
    unsigned char count)
{
	...
	for(i = 0; i < count; i++) {
		...
		if(data_frame->THL == current.THL && data_frame->origin == current.origin &&
...
void cache_remove(unsigned char offset, node_state *state)
{
	...
	if(!offset) {
		...
		for(i = offset; i < state->output_cache_count; i++) {
			memcpy(&state->output_cache[(offset + i) % CACHE_SIZE],
			    &state->output_cache[(offset + i + 1) % CACHE_SIZE], sizeof(ctp_data_packet));
		}
	}
...
void schedule_retransmission(node_state *state)
{
	...
	double interval =
	    (RandomRange((unsigned int)(data_packet_transmission_delta * 1000),
		(unsigned int)(((data_packet_transmission_delta + data_packet_transmission_offset) * 1000) - 1))) /
	    1000.0;
 ...
 * engine relies on the routing engine for a few pieces of information related to the path of the packet to be
 * forwarded.
 *
	...
	if(!result) {
		...
		if(compare_data_packets(&head->data_packet_frame, &state->last_packet_acked.data_packet_frame,
			...
			if(head_entry->retries) {
				...
				/*printf("Node %d -> packet dropped ->before dequeueing head was %d and state was
				%d\n",state->me, state->forwarding_queue_head,state->state); printf("Entry was
				local?%d\n",head_entry->is_local); printf("TIME:%f\n",state->lvt);
				printf("//////////\n");
				fflush(stdout);*/
				forwarding_queue_dequeue(state);
