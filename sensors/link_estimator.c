 ...
 * FIND WORST ENTRY
 *
 * Returns the index of the entry in the table with the highest ETX which is greater than or equal to the given
 * threshold This routine gets called when a new entry has to be initialized in the neighbor table and there are no free
 * entries left => it helps deciding whether the new entry has to replace an existing one and, in this case, which of
 * the existing entries has to be evicted
 *
 ...
 * returned.
 * This routine gets called when it is necessary to evict an entry of the neighbor table and known of the existing
 * entries is really worse than the other ones => choose randomly
 *
 * @link_estimator_table: pointer to the link estimator table of the node
 */
...
unsigned char find_random_entry(link_estimator_table_entry *link_estimator_table)
{
	...
	for(i = 0; i < NEIGHBOR_TABLE_SIZE; i++) {
		...
		if(link_estimator_table[i].flags & VALID_ENTRY) {
			...
			if((link_estimator_table[i].flags & MATURE_ENTRY) ||
			    (link_estimator_table[i].flags & PINNED_ENTRY)) {
			} else {
				/*
...
int insert_neighbor(unsigned int neighbor, link_estimator_table_entry *link_estimator_table)
{
	...
	if(index == INVALID_ENTRY) {
		...
		if(index != INVALID_ENTRY) {
			...
			index =
			    find_estimator_worst_entry((unsigned char)evict_best_etx_threshold, link_estimator_table);
	...
	for(i = 0; i < NEIGHBOR_TABLE_SIZE; i++) {
		...
		if(entry->neighbor == neighbor) {
			...
			if(entry->flags & VALID_ENTRY) {
				...
				entry->ingoing_quality =
				    (alpha * entry->ingoing_quality + (10 - alpha) * new_ingoing_quality) / 10;
...
void update_neighbor_entry(unsigned char index, unsigned char seq, link_estimator_table_entry *link_estimator_table)
{
	...
	if(lost_beacons > max_pkt_gap) {
		...
		if((link_estimator_table[index].beacons_missed + link_estimator_table[index].beacons_received >=
		       blq_pkt_window) ||
    ...
    ctp_routing_frame *routing_frame, node_state *state)
{
	...
	if(link_frame->sink == BROADCAST_ADDRESS) {
		...
		if(index != INVALID_ENTRY) {
			...
			/*
			 * No entry corresponding to the sender has been found => this is the first message received
			 * from him and a corresponding entry has to be created => look for a free entry in the neighbor
			 * table
			 */
 ...
 * the message. The LINK estimator is in charge of extracting the physical and link estimator frames and use the
 * contained pieces of information to keep the neighbor table up to date. Then message is then passed to the upper layer
 * of the CTP stack, the ROUTINE ENGINE, to be further processed
 *
