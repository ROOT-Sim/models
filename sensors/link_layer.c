 ...
 * In particular the node has to "see" the channel free for a number of consecutive times equal to
 * CSMA_MAX_FREE_SAMPLES: as soon as this holds, or when CSMA_MAX_FREE_SAMPLES is set to zero, the node starts to
 * transmit the frame. In case the node sees that the channel is busy, it backs off, i.e. it recomputes the value of the
 * backoff time and schedules a new check when such a time has elapsed (unless the maximum number of backoffs has
...
void check_channel(node_state *state)
{
	...
	if(!state->free_channel_count) {
		...
		simtime_t backoff = RandomRange(0,
		    (unsigned int)((csma_high - csma_low) * pow(csma_exponent_base, state->backoff_count)));
...
void frame_transmitted(node_state *state)
{
	...
	printf("Fields of last link frame
