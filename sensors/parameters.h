 ...
 * FAILURE LAMBDA
 *
 * The lambda parameter of the exponential failure distribution: it depends on the devices used as node of the
 * collection tree and is equivalent to the failure rate or MTTF(Mean Time To Failure)
 */
...
extern double max_simulation_time, failure_lambda, failure_threshold;

extern double white_noise_mean, channel_free_threshold;

extern unsigned csma_symbols_per_sec, csma_bits_per_symbol, csma_min_free_samples, csma_max_free_samples, csma_high,
    csma_low, csma_init_high, csma_init_low, csma_rxtx_delay, csma_exponent_base, csma_preamble_length, csma_ack_time;
extern double csma_sensitivity;

extern unsigned evict_worst_etx_threshold, evict_best_etx_threshold, max_pkt_gap;
extern unsigned short alpha, dlq_pkt_window, blq_pkt_window;

extern double update_route_timer, min_beacons_send_interval, max_beacons_send_interval;
extern unsigned max_one_hop_etx, parent_switch_threshold;

extern unsigned max_retries, min_payload, max_payload;
extern double data_packet_transmission_offset, data_packet_transmission_delta, no_route_offset, send_packet_timer,
    create_packet_timer;
