 ...
 *
 * Because of the limited GAIN of the radio featured by the nodes and because of the NOISE associated to the
 * environment, a packet sent from one node to another can't always be received => before a packet is delivered to
 * another node, i.e. it receives an event, the simulation has to check that the packet can actually be received by the
 * recipient and this module performs this check. The interferences of the environment (noise) are simulated using an
 * additive signal strength model: the power of the signal through the channel is the sum of the gain associated to all
 * the packets sent and not yet received in the network => a channel is considered free only if the total power of the
 * signal representing such a noise is below a threshold.
 */
 ...
 * white gaussian noise. The latter is modelled as a gaussian random variable, having mean 0 and whose standard
 * deviation is read from the input file, together with the value of the noise floor.
 */
	...
	if(a->frame_type == b->frame_type && a->lost == b->lost && a->power == b->power) {
		...
		if(a->frame_type == CTP_BEACON) {
			...
			return compare_link_layer_frames(&data_packet_a->link_frame, &data_packet_b->link_frame) &&
			       compare_data_packets(&data_packet_a->data_packet_frame,
				   &data_packet_b->data_packet_frame, data_packet_a->payload, data_packet_b->payload);
		}
	}
...
void new_pending_transmission(node_state *state, double gain, unsigned char type, void *frame, double duration)
{
	...
	if(state->state & RUNNING) {
		...
		if(channel_strength + csma_sensitivity < gain) {
			...
			if(!(state->radio_state & RADIO_RECEIVING) && !(state->radio_state & RADIO_TRANSMITTING)) {
				/*
...
void transmission_finished(node_state *state, pending_transmission *finished_transmission)
{
	...
	if(!finished_transmission->lost) {
		...
		/*
		 * Inform the LINK ESTIMATOR or the FORWARDING ENGINE about the reception: check the type to determine
		 * if the frame contains a beacon or a data packet
		 */
