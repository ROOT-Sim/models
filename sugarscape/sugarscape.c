...
{
	unsigned *info_p =
	    NULL; // this points to 2 consecutive unsigned ints, the sugar and the eaters in the region respectively
	...
	while(i--) {
		if(GetNeighbourInfo(i, &receiver, (void **)&info_p) != 1 && info_p && !info_p[1] &&
		    info_p[0] > max_sugar)
			max_sugar = info_p[0];
	}
	// decrement eaters
...
int OnGVT(unsigned int me, region_t *snapshot)
{
	if(snapshot->n.eaters != 0)
		printf("%u: cap %u eaters %u sugar %u\n", me, snapshot->capacity, snapshot->n.eaters,
		    snapshot->n.sugar);
