...
void ProcessEvent(unsigned int me, simtime_t now, int event_type, void *event, size_t size, struct state *state) {
    ...
    int receiver;
    //int i;
    struct vehicle *event_content = (event_content *) event;
    ...
    switch (event_type) {
        ...
        case LP_INIT:
            ...
            if (me == 0)
                printf("STARTING TRAFFIC WITH END TIME: %d\n input rate: %f\n output prob: %f\n\n", EXECUTION_TIME,
                       state->enter_prob, state->leave_prob);
        ...
        case LEAVE:
            ...
            if (car != NULL) {
                ...
                ScheduleNewEvent(receiver, /*LOOKAHEAD +*/ car->leave, ARRIVAL, &new_event, sizeof(event_content));
                free(car);
            } else {
                timestamp = now + Expent(ACCIDENT_LEAVE_TIME);
                update_car_leave(state, *(unsigned long long *) event_content, timestamp);
                ScheduleNewEvent(me, /*LOOKAHEAD +*/ timestamp, LEAVE, (unsigned long long *) event_content,
