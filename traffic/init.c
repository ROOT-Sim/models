...
static bool
parse_topology_line(unsigned int me, unsigned int *line_counter, char *line, int *state, lp_state_type *sim_state,
                    FILE *f) {
    char to[NAME_LENGTH];
...
void init_my_state(int me, lp_state_type *sim_state) {
    ...
    printf("LP %d (%s) is a%s of length %f with ", me, sim_state->name,
           (sim_state->lp_type == JUNCTION ? "n intersection" : " route"), sim_state->segment_length);
    ...
    for (i = 0; i < sim_state->topology->num_neighbours; i++) {
        ...
        if (sim_state->topology->neighbours[i] >= n_lps) {
