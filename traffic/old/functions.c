...
void inject_new_cars(lp_state_type *state, int me) {
    ...
    new_evt.from = me;
    new_evt.injection = 1;
    ScheduleNewEvent(me, timestamp, ARRIVAL, &new_evt, sizeof(event_content));
}
...
void cause_accident(lp_state_type *state, int me) {
    ...
    mean = (double) state->total_queue_slots /
           3.0; // When there are many cars but not that much, accidents are more likely to occur
...
car_t *car_dequeue(unsigned int me, lp_state_type *state, unsigned long long *mark) {
    ...
    while (curr_car->next != NULL && curr_car->next->car_id != *mark) {
        ...
        curr_car = curr_car->next;
        curr_car->speed = fabs(
                Gaussian(state->segment_length / (curr_car->leave - curr_car->arrival), SPEED_SIGMA)); //TODO
    }
