...
static void reorder_queue(lp_id_t me, struct vehicle **head, struct state *state)
{
	...
	curr = *head;
	while(curr != NULL) {
		simtime_t total_time = curr->leave_time - curr->arrival_time;
		if(total_time > 0.0) {
			curr->traveled = (now - curr->arrival_time) / total_time;
			if(curr->traveled < 0.0) curr->traveled = 0.0;
			if(curr->traveled > 1.0) curr->traveled = 1.0;
		} else {
			curr->traveled = 0.0;
		}
		curr = curr->next;
...
void process_car_leave(lp_id_t me, struct state *state)
{
	...
	while(*curr_car_ptr != NULL && (*curr_car_ptr)->leave_time <= state->lvt + 1e-6) {
		...
		*curr_car_ptr = dequeued_car->next;
		...
		if(IS_JUNCTION(me)) {
			...
			unsigned long neighbours = count_neighbours(me);
			if(neighbours == 0) {
			...
			if(neighbours > 1) {
				int retries = 0;
				while(new_event.to == dequeued_car->from && retries < 10) {
					new_event.to = get_random_destination(me);
					retries++;
				}
			}
	...
	if(state->queue != NULL) {
		simtime_t next_leave = state->queue->leave_time;
		if(next_leave < state->lvt) {
			next_leave = state->lvt;
		}
		ScheduleNewEvent(me, next_leave, LEAVE, NULL, 0);
	}
}
