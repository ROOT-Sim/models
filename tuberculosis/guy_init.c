...
void guy_init(void)
{
	...
	while(remaining) {
		...
		// printf("\nrequested i:%u s:%u t:%u f:%u h:%u\n", to_send.infected, to_send.sick, to_send.treated,
		// to_send.treatment, to_send.healthy); fflush(stdout);
...
static void new_treated(void)
{
	...
	// (notice that this is equivalent to what is done in the original model)
	guy->treatment_day =
	    Random() < p_abandon ? 0 - RandomRange(t_treatment_min, t_treatment_max - 1) : 0 - (int)t_treatment_max;
