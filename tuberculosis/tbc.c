...
static void move_healthy_people(unsigned me, region_t *region, simtime_t now)
{
	...
	// we do this in 2 rounds to minimize the effect of the bias of the binomial generator xxx: is it needed?
	// Perform some tests!
...
void ProcessEvent(
    unsigned int me, simtime_t now, int event_type,
    union {
	    agent_t *agent;
	    unsigned *n;
	    infection_t *i_m;
    ...
    unsigned int event_size, region_t *state)
{
